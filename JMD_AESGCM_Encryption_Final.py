...
def counter(N):
    ...
    N1=N+Ctr
    return N1
def hexad(N):# binary(string) to hex conversion
    t=0
    out=[]
    while t<len(N):
        M=N[t:t+4]
...
def part(N):# in:string hex i.e. 'cef', out:list-str binary i.e.[['1','0','1'],['1','0','1']]
    i=j=0
    N1=[]
    while i<(len(N)/32):
        temp=''.join(N[j:j+32])
        temp=list(binary(temp))
        N1.append(temp)
        i+=1
        j+=32
    if ((len(N)%32)==0):
...
def right(N):
    ...
    N.append('0')
    return N
def mult(N,N1):#Z=N*N1(128 bit list-str)
    N=reverse(N)
#    Xm=reverse(Xm)
    Z=list(binary('00000000000000000000000000000000'))
    R=list(binary('00000000000000000000000000000000'))
    R[7]=R[2]=R[1]=R[0]='1'
    R=reverse(R)
    Vm=N
...
def GHASH(H,A,C):
    ...
    if A=='':
        ...
        Alen=padd(hex(len(A)*4).split('x')[1])
        A=part(A)
#    print A
        i=0
        while i<len(A):
    ...
    if C=='':
        ...
        Clen=padd(hex(len(C)*4).split('x')[1])
#        print 'at 1:',Clen
        Ctemp=part(C)
#        print 'at 2:',Clen
#    print A
...
#print last
    X=xor(X,last[0])
    X=mult(X,H)
    print 'len(A)||len(C):',Alen+Clen
    return X
K=raw_input('Enter the Key:')
P=raw_input('Enter the P:')
A1=raw_input('Enter the A:')
IV=raw_input('Enter the IV:')
zero='00000000000000000000000000000000'
X=list(binary(zero))
H1=aes(zero,K)
print 'H:',H1
...
if P=='':
    ...
    Pdash=part(P)
#    print Pdash
#    print 'Pdash:',hexad(''.join(Pdash[len(Pdash)-1]))
    C1=[]
    i=1
    j=0
    while j<len(Pdash):
        Y=inc(Y)
        print 'Y',i,':',Y
        dash=aes(Y,K)#out: 32 hex-string
        print 'E( K,Y',i,'):',dash
        dash=part(dash)
        temp=xor(dash[0],Pdash[j])
        C1.append(hexad(temp))
        j+=1
        i+=1
#    print C1
    if len(P)%32==0:
        ...
        scall=len(C1)-1
        Ctemp=C1[scall]
        Ctemp=Ctemp[0:len(P[(32*(len(P)/32)):])]
        del C1[scall]
        C1.insert(scall,Ctemp)
    C1=''.join(C1)
#    print C1
X1=GHASH(H1,A1,C1)
Tag=xor(X1,Ysave[0])
Tag=hexad(Tag)
print 'GHASH(H,A,C):',hexad(X1)
print 'C:',C1
print 'T:',Tag
